...
export default function ReportsTab({ employees, organizationId }) {
  ...
  const shareViaEmail = () => {
    const csvContent = generateCSV()
    const subject = 'V.A.S Tri State Time Tracking Report'
...
Here's the V.A.S Tri State time tracking report for ${startDate} to ${endDate}:
...
Generated by V.A.S Tri State Time Tracking System
Powered by VasHours - Visit: https://vashours.vercel.app

Best regards,
V.A.S Tri State Team`
  ...
  const generatePDF = async () => {
    ...
    pdf.text('V.A.S Tri State', 20, 18)
    ...
    for (let i = 1; i <= pageCount; i++) {
      ...
      pdf.text('Generated by V.A.S Tri State - Powered by VasHours', 105, 285, { align: 'center' })
    }
    
      // Download the PDF
      const fileName = `VAS-TriState-Report-${startDate}-to-${endDate}.pdf`
