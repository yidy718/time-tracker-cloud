...
export const database = {
  ...
  getCurrentEmployee: async (userId) => {
    ...
    // If not found and no error, try to find by matching auth user email (for admin logins)
    if (!data && !error) {
      console.log('Employee not found by ID, trying to match by email...')
      
      // Get the auth user's email
      const { data: authUser } = await supabase.auth.getUser()
      if (authUser?.user?.email) {
        const { data: employeeByEmail, error: emailError } = await supabase.from('employees').select(`
          *,
          organization:organizations(name)
        `).eq('email', authUser.user.email).eq('is_active', true).maybeSingle()
        
        if (employeeByEmail) {
          data = employeeByEmail
          error = emailError
        } else {
          // If this is a known admin email but no employee record exists, create a virtual admin user
          if (authUser.user.email === 'yidy.brier@gmail.com') {
            console.log('Creating virtual admin user for:', authUser.user.email)
            data = {
              id: '882247fb-71f2-4d1d-8cfd-f33d8c5a3b0f', // Use the known admin ID
              email: authUser.user.email,
              first_name: 'Yidy',
              last_name: 'Breuer',
              role: 'admin',
              is_active: true,
              organization_id: '59590af0-3c38-403e-b4c9-ef4ba8db2a0b', // Default org
  ...
  getProjectLocations: async (projectId) => {
    try {
      const { data, error } = await supabase
        .from('project_locations')
        .select(`
      ...
      console.error('getProjectLocations failed:', error)
      return { data: [], error }
    }
  },
