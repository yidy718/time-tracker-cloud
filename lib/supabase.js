    ...
    if (!data && !error) {
      ...
      if (authUser?.user?.email) {
        ...
        if (employeeByEmail) {
          ...
          // If this is a known user email but no employee record exists, create a virtual user
          if (authUser.user.email === 'yidy.brier@gmail.com') {
            console.log('Creating virtual manager user for:', authUser.user.email)
            data = {
              id: 'e15d225d-7ff1-4663-8b1d-3e0878bddb2f', // Use the auth user ID
