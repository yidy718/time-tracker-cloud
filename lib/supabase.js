...
export const database = {
  ...
  createEmployee: async (employee) => {
    try {
      // Try to create employee with regular permissions first
      const { data, error } = await supabase
        .from('employees')
        .insert(employee)
        .select()
      ...
      if (error) {
        console.error('Regular insert failed:', error)
        
        // If RLS error, try using RPC function with admin privileges
        if (error.message.includes('row-level security') || error.message.includes('violates row-level security')) {
          console.log('Attempting RPC insert due to RLS violation...')
          
          // Use RPC function that bypasses RLS for admin users
          const { data: rpcData, error: rpcError } = await supabase
            .rpc('admin_create_employee', {
      ...
      console.error('CreateEmployee function error:', err)
    ...
    try {
      let query = supabase
        ...
        .select(`
          ...
          po_number,
          invoice_number,
          assigned_employee:employees!task_details_assigned_employee_id_fkey(hourly_rate)
        `)
        .eq('organization_id', organizationId)
        .eq('status', 'completed')
