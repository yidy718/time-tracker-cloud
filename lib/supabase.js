...
export const database = {
  ...
  authenticateEmployee: async (username, password) => {
    // SECURITY WARNING: This authentication method may use plain text passwords
    // TODO: Implement proper password hashing (bcrypt) in database and authentication function
    // For production use, passwords should NEVER be stored or compared in plain text
    
    console.log('Trying authentication function for:', username)
    
    // Basic input validation to prevent injection attacks
    if (!username || !password || username.length < 3 || password.length < 6) {
      return { data: null, error: new Error('Invalid username or password format') }
    }
    
    // Sanitize username input to prevent SQL injection
    const sanitizedUsername = username.replace(/[^a-zA-Z0-9_.-]/g, '')
    if (sanitizedUsername !== username) {
      return { data: null, error: new Error('Invalid characters in username') }
    }
    ...
    const { data, error } = await supabase
      .rpc('authenticate_employee', {
        ...
        input_password: password // WARNING: This should be hashed
      })
    ...
    if (serviceRoleKey) {
      ...
      // WARNING: This query compares plain text passwords - major security risk
      const { data: serviceData, error: serviceError } = await serviceClient
        .from('employees')
        .select('*')
        .eq('username', sanitizedUsername)
        .eq('password', password) // CRITICAL: This should use password hashing
        .eq('is_active', true)
        .single()
    ...
    return { data: basicData, error: basicError || new Error('Authentication failed') }
  },
  // Organizations
  getOrganization: (id) =>
  ...
  deleteClientProject: (id) =>
    supabase.from('client_projects').delete().eq('id', id),
