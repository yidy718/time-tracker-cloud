...
export const database = {
  ...
  getClientProjects: async (organizationId) => {
    try {
      ...
      console.warn('client_projects table not found or accessible:', error)
  ...
  createClientProject: async (project) => {
    try {
      console.log('Creating client project:', project)
      const { data, error } = await supabase
        .from('client_projects')
        .insert(project)
        .select()
      ...
      console.log('Client project created successfully:', data)
      return { data, error: null }
    } catch (error) {
      console.error('createClientProject failed:', error)
  ...
  updateClientProject: async (id, updates) => {
    try {
      console.log('Updating client project:', { id, updates })
      const { data, error } = await supabase
        .from('client_projects')
        .update(updates)
        .eq('id', id)
      ...
      console.log('Client project updated successfully:', data)
      return { data, error: null }
    } catch (error) {
      console.error('updateClientProject failed:', error)
      return { data: null, error }
    }
  },
  ...
  deleteClientProject: async (id) => {
    try {
      console.log('Deleting client project:', id)
      ...
      console.log('Client project deleted successfully')
      return { error: null }
    } catch (error) {
      console.error('deleteClientProject failed:', error)
      return { error }
    }
  },
  ...
  clockIn: async (employeeId, locationId, projectId = null) => {
    try {
      ...
      console.warn('Clock in with project_id failed, trying without:', error)
      
      // Fallback without project_id if column doesn't exist
      try {
        const { data, error } = await supabase.from('time_sessions').insert({
          employee_id: employeeId,
          location_id: locationId,
    ...
    try {
      let query = supabase
        ...
        .select(`
          *,
          ...
          location:locations(name),
          project:client_projects(project_name, name)
        `)
      ...
      const processedData = data.map(session => {
        ...
        return {
          ...
          project_name: session.project?.project_name || session.project?.name
        }
      })
      ...
      const processedData = data.map(session => {
        ...
        return {
          ...
          hourly_rate: session.employee.hourly_rate,
          location_name: session.location?.name,
          project_name: null // No project data available
        }
      })
  ...
  getAllTimeSessions: async (organizationId, startDate, endDate) => {
    ...
    try {
      ...
      const processedData = data.map(session => ({
        ...
        project_name: session.project?.project_name || session.project?.name || 'No Project'
      }))
      ...
      const processedData = data.map(session => ({
        ...
        last_name: session.employee.last_name,
        location_name: session.location?.name || 'Unknown',
        project_name: 'No Project' // No project data available
      }))
