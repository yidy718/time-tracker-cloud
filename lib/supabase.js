...
export const database = {
  ...
  getClientProjects: async (organizationId) => {
    console.log('getClientProjects called for organization:', organizationId)
    try {
      const { data, error } = await supabase
        .from('client_projects')
        .select('id, client_name, project_name, project_code, billing_rate, is_active')
        .eq('organization_id', organizationId)
        .eq('is_active', true)
      ...
      console.log('getClientProjects success:', data)
      return { data: data || [], error: null }
    } catch (error) {
      console.error('getClientProjects failed:', error)
