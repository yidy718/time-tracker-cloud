-- Multi-Company Assignment and Enhanced Roles Migration (Fixed)
-- This migration adds multi-company support, new roles, and expenses feature

-- 1. First, drop the existing function to allow modification
DROP FUNCTION IF EXISTS authenticate_employee(text, text);

-- 2. Drop existing policies that depend on the role column
DROP POLICY IF EXISTS "Admins can manage all time sessions" ON time_sessions;
DROP POLICY IF EXISTS "Admins can manage locations" ON locations;
DROP POLICY IF EXISTS "Auth users can manage own org projects" ON projects;
DROP POLICY IF EXISTS "Admins can manage project locations in their organization" ON project_locations;
DROP POLICY IF EXISTS "Admins can manage task assignments in their organization" ON task_assignments;
DROP POLICY IF EXISTS "Admins can manage all tasks in their organization" ON tasks;

-- 3. Add a temporary role column with new type
ALTER TABLE employees ADD COLUMN role_new employee_role;

-- 4. Copy existing role values, mapping old values to new enum
UPDATE employees SET role_new = 
  CASE 
    WHEN role = 'admin' THEN 'admin'::employee_role
    WHEN role = 'employee' THEN 'employee'::employee_role
    WHEN role = 'manager' THEN 'manager'::employee_role
    ELSE 'employee'::employee_role
  END;

-- 5. Drop the old role column and rename the new one
ALTER TABLE employees DROP COLUMN role;
ALTER TABLE employees RENAME COLUMN role_new TO role;

-- 6. Set default and not null constraint
ALTER TABLE employees ALTER COLUMN role SET NOT NULL;
ALTER TABLE employees ALTER COLUMN role SET DEFAULT 'employee'::employee_role;

-- 7. Create user_organizations table for multi-company assignment
...
-- 8. Add enable_expenses to organizations table
...
-- 12. RLS Policies for user_organizations
...
-- 14. Function to get user's organizations
...
-- 15. Function to assign user to organization
...
BEGIN
    ...
    IF assigner_role IS NULL OR assigner_role != 'admin' THEN
        RAISE EXCEPTION 'Insufficient permissions to assign users to organizations';
    END IF;
    
    -- Insert or update assignment
    INSERT INTO user_organizations (user_id, employee_id, organization_id, role)
...
-- 16. Migrate existing employees to user_organizations table
...
BEGIN
    ...
    LOOP
        ...
        IF NOT EXISTS (
            ...
            -- Create entry for backwards compatibility
...
-- 17. Update authenticate_employee function to work with multi-company
...
-- 18. Recreate the essential RLS policies
